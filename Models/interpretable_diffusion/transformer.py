...
class Transformer(nn.Module):
    ...
    def forward(self, input, t, padding_masks=None, return_res=False):
        emb = self.emb(input)  # 加噪的x--卷积  128 24 64  t 128
        inp_enc = self.pos_enc(emb)  #  加噪的x--卷积+加位置enc
        enc_cond = self.encoder(inp_enc, t, padding_masks=padding_masks)  # 加噪的x--卷积+加位置enc+  时间轴加位置信息通过fullattention融合在一起 128 24 64 -- 条件

        inp_dec = self.pos_dec(emb)  # 输入经过特征提取的部分  加噪的x--卷积--位置编码
        output, mean, trend, season = self.decoder(inp_dec, t, enc_cond, padding_masks=padding_masks)  #  加噪的x--卷积--位置编码=inp_dec ==Q  加噪的x--卷积+加位置enc+ encoder ==enc_cond==KV  128 24 64
