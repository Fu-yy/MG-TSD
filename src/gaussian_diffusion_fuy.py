...
class GaussianDiffusion(nn.Module):
    ...
    def log_prob(self, x, cond, *args, **kwargs):
        ...
        return loss
class MyDenoiseNet(nn.Module):
    def __init__(self, input_dim):
        super(MyDenoiseNet, self).__init__()
...
class Model(nn.Module):
    def __init__(self, input_dim):
        ...
        self.denoisemodel = MyDenoiseNet(96)
        self.gaussiandiff = GaussianDiffusion(denoise_fn=self.denoisemodel,
        input_size=96,)
    def forward(self,x):
        batch_size, seq_len, nvar = x.shape

        t = torch.randint(0, self.gaussiandiff.num_timesteps, (x.shape[0],), device=x.device)  # 随机时间步
        loss = self.gaussiandiff.p_losses(x, cond=None, t=t, noise=None)
        sampled_data = self.gaussiandiff.sample(sample_shape=x)

        return sampled_data,loss

if __name__ == '__main__':
    x = torch.randn(32,96,7)
    m = Model(input_dim=96)
    sampled_data,loss = m(x)
    c = '1'