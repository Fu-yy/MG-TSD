...
class GaussianDiffusion(nn.Module):
    ...
    def log_prob(self, x, cond, *args, **kwargs):
        ...
        # )
        time_step = torch.randint(0, self.num_timesteps, (B,), device=x.device).long()

        # t_base  = torch.randint(low=0, high=self.num_timesteps, size=(B // 2 + 1,))
        # time_step = torch.cat([t_base, self.num_timesteps - t_base - 1], dim=0)[:B].to(x.device)
        loss = self.p_losses(
            x, cond, time_step, *args, **kwargs
        )
