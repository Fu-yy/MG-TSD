...
class UpsampleBlock(nn.Module):
    def __init__(self, up_factor=2, in_dim=137, out_dim=137,in_pad=4,out_pad=0):
        super().__init__()
...
class ProgressiveDenoisingModel(nn.Module):
    def __init__(
        ...
        self.upsamplers = nn.ModuleList([
            UpsampleBlock(up_factor=downsample_factor, in_dim=target_dim // (downsample_factor ** (i+1)),in_pad=4,out_pad=4)
    ...
    def forward(self, x, time, cond):
        ...
        # 残差层
        for layer in self.stages[-1]:
            _, skip = layer(denoised, cond_up, diffusion_step)
            # 跳跃连接投影
